...
export const getTeamTasks = async (projectId, userId) => {
    const project = await prisma.project.findUnique({
        where: { id: projectId },
        select: { teamId: true },
    });
...
export const assignTask = async (taskId, teamId, assigneeId, actorId, overwrite = false) => {
    ...
    if (existingTask.assigneeId &&
        ...
        !overwrite) {
        throw new Error(`Task is already assigned to ${existingTask.assignee?.name}. Confirm overwrite.`);
    }
    try {
        ...
        throw new Error("Target user is not a member of this team and cannot be assigned tasks.");
    }
    // Perform update
    const updatedTask = await prisma.task.update({
        where: { id: taskId },
...
export const updateTaskStatus = async (taskId, newStatus, userId) => {
    ...
    if (!allowedMoves.includes(newStatus)) {
        throw new Error(`Invalid transition: Cannot move task from ${task.status} to ${newStatus}`);
    }
    const updatedTask = await prisma.task.update({
        where: { id: taskId },
