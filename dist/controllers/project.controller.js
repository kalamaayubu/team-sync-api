...
export const getProjectByIdHandler = async (req, res) => {
    try {
        ...
        const project = await ProjectService.getProjectById(teamId, req.user?.id, projectId);
        if (!project) {
...
export const deleteProjectByIdHandler = async (req, res) => {
    try {
        ...
        await ProjectService.deleteProjectById(projectId, req.user.id, teamId);
        return res.status(200).json({
            success: true,
        ...
        const isClientError = error.message.includes("permission") ||
